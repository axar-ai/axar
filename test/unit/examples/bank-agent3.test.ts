...
import { SupportAgent, DatabaseConn } from '../../../src/examples/bank-agent3'; // Adjust import path as needed
import z from 'zod';
...
describe('SupportAgent', () => {
  ...
  it('should handle simple queries and validate response schema', async () => {
    ...
    const parsed = z
      .object({
        support_advice: z.string(),
        block_card: z.boolean(),
        risk: z.number().min(0).max(1),
        status: z.enum(['Happy', 'Sad', 'Neutral']).optional(),
      })
      .safeParse(result);
